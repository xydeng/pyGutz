...
#  Following the convention in Wannier90, H(R) is based on unit cell,  not on orbitals.
#  H(k)=sum_R H(R) exp(-ikR)
#
#  A tight-binding model is defined by a set of hoppoing matrix H(R). H(R) is a matrix with dimension of the number of orbitals in the unit cell.
#
#  A usefull technique is to construct supercell, which needs to construct H(R) for the supercell
...
class AtomsTB(Atoms):
    ...
    Attributes:
      Attributes of ASE ATOMS object; check documents
...
class TB(object):
    ...
    def eigens(self,kps=numpy.zeros((1,3)),keepwf=False):
        ...
        Returns:
          (eks,Uk) or (eks,): eks, eigenvalues, taken to be real;
          Uk, wavefunctions. Both are sorted according to the order of
          eks.
...
if __name__=="__main__":
    ...
    # construct TB in Nambu basis
    ## add spin degeneracy and transform to Nambu basis.
